...
class Applier:
    ...
    def rename_spots(self):
        ...
        for obj in self.pdf.objects:
            ...
            self.report[f"spot renamed: {src} -> {target}"] += 1
    ...
    def _remap_container(self, container):
        ...
        for operands, op in parsed:
            ...
            if hit:
                ...
                self.report[f"remap {kind}: "
                            f"{'/'.join(str(round(c*100, 1)).rstrip('0').rstrip('.') for c in cmyk)}"
                            f" -> {spot}"] += 1
                changed = True
            else:
...
def format_report(report, notes, applied):
    lines = []
...
def cli(argv):
    ...
    print(format_report(report, notes, applied=bool(out)))
    if out:
        print(f"Wrote {out}")
...
def gui():
    from PySide6.QtWidgets import (
        ...
        QMainWindow, QMessageBox, QPlainTextEdit, QPushButton, QWidget)
    ...
    class Win(QMainWindow):
        ...
        def _run(self, dry):
            ...
            try:
                ...
                return
            text = format_report(report, notes, applied=bool(out))
            if out:
                text += f"\nWrote {out}"
